...
{
	cout << "※※动力定位控制仿真※※" << endl << endl << endl;

	cout << "★请设置环境参数：" << endl;

	cout << "\t风速(默认为" << SpeedWind << "m/s) = ";
	...
	do 
	{
		...
		{
			cout << "\t¤¤输入有误，请确认后再次输入！" << endl;
		}
	} while (1 != dpFlag && 2 != dpFlag);

	cout << "\n★请选择动力定位控制器类型(1.PID控制器；2.非线性模型预测控制器)：" << endl;

	do 
	{
		cout << "\t默认控制器类型为 1.PID控制器。\n\t请输入控制器类型代码： ";
	...
	cout << "\n★请选择环境最优动力定位控制策略\n"
		<< "\t(1.WOPC与ZPC-W结合后的环境最优动力定位控制策略;\n"
		<< "\t2.WOPC借用环境估计的环境最优动力定位控制策略;\n"
		<< "\t3.ZPC-W环境最优动力定位控制策略)：" << endl;

	do 
	{
		cout << "\t默认控制策略为 1.WOPC与ZPC-W结合后的环境最优动力定位控制策略。\n"
			<< "\t请输入控制策略代码： ";
	...
	paraFile << "★动力定位控制器类型★" << endl;
	switch (dpFlag)
	{
	case 1:
		paraFile << "\tPID控制器" << endl << endl;
		paraFile << "★PID控制器参数★\n";
		paraFile << "\tKp = " << kp << endl;
		paraFile << "\tKi = " << ki << endl;
		paraFile << "\tKd = " << kd << endl;
	...
	if (2 == dpFlag)
	{
		...
		{
		case 1:
			paraFile << "★ZPC-W环境最优艏向控制器的参数★" << endl;
